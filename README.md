...
How to define a service route for header & sticky session based routing using ingress, virtual Service and destination rule.
...
2. These 3 backend should be hosted on k8s setup within minikube/localhost i.e
		app1.py -> backend-1
		app2.py -> backend-2
		app3.py -> backend-3
...
3. A loadbalancer with k8s ingress or custom nginx/haproxy ingress controller that route /hit_backend traffic based on the following routing rules:
    Rule-1:  Use "target" field in request body or request header to route to corresponding backend (i.e: "target":"backend-1" route to backend-1 service)
